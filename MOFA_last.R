...
model_scaled = load_model("model_TMM.hdf5",remove_inactive_factors = FALSE)
...
sample_metadata$gender= metadata_new$Gender
...
plot_factors(model_scaled, 
             factors = 1:5,
             color_by = "sample.type"
)

plot_weights(model_scaled,
...
plot_data_heatmap(model_scaled,
...
plot_data_scatter(model_scaled,
                  view = "Transcriptomics",         # view of interest
                  factor = 1,             # factor of interest
                  features = 20,           # number of features to plot (they are selected by weight)
...
top_100_positive_Methylation <- head(df_methyl_1[order(df_methyl_1$value, decreasing = TRUE), ], 100)
write.csv(top_100_positive_Methylation,"Methylatione_top_100_pos.csv")
top_100_negative_Methylation <- head(df_methyl_1[order(df_methyl_1$value, decreasing = FALSE), ], 100)
write.csv(top_100_negative_Methylation,"Methylatione_top_100_neg.csv")

df_Trans_1=get_weights(
  model_scaled,
...
top_100_positive_Trans <- head(df_Trans_1[order(df_Trans_1$value, decreasing = TRUE), ], 100)
write.csv(top_100_positive_Trans,"top_100_positive_Trans.csv")
top_100_negative_Trans <- head(df_Trans_1[order(df_Trans_1$value, decreasing = FALSE), ], 100)
write.csv(top_100_negative_Trans,"top_100_negative_Trans.csv")
...
top_100_positive_Methylation <- head(df_methyl_1[order(df_methyl_1$value, decreasing = TRUE), ], 100)
write.csv(top_100_positive_Methylation,"Methylatione_top_100_pos_fac3.csv")
top_100_negative_Methylation <- head(df_methyl_1[order(df_methyl_1$value, decreasing = FALSE), ], 100)
write.csv(top_100_negative_Methylation,"Methylatione_top_100_neg_fac3.csv")
...
top_100_positive_Trans <- head(df_Trans_1[order(df_Trans_1$value, decreasing = TRUE), ], 100)
write.csv(top_100_positive_Trans,"top_100_positive_Trans_fac2.csv")
top_100_negative_Trans <- head(df_Trans_1[order(df_Trans_1$value, decreasing = FALSE), ], 100)
write.csv(top_100_negative_Trans,"top_100_negative_Trans_fac2.csv")
